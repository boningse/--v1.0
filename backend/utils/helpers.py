...
CONVERSION_FACTORS = {
    ...
    3: {"name": "原始数据", "unit": "kWh", "factor": 1.0},
}

def apply_conversion(data_value: float, conversion_type: int = 3) -> float:
    info = CONVERSION_FACTORS.get(conversion_type, CONVERSION_FACTORS[3])
    return round(data_value * info["factor"], 4)

def get_conversion_info(conversion_type: int = 3) -> dict:
    return CONVERSION_FACTORS.get(conversion_type, CONVERSION_FACTORS[3])
