...
from utils.helpers import tenement_table, get_year_months, parse_date, safe_float
...
async def tenement_analysis(
    ...
    xdate: str = Query("day"), energy_type: int = Query(1), user: dict = Depends(current_user),
):
    """分户分析数据 - 对应 M=1 (时序图)"""
    ...
    result = []
    for t in tenements:
        item_data = [round(time_data[ts].get(t["sign"], 0), 2) for ts in times]
