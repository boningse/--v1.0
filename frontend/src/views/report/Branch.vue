  ...
  <div class="report-page">
    ...
    <div id="printArea" class="report-content" style="margin-top:12px">
      ...
      <el-table :data="tableData" border stripe size="small" v-loading="loading" style="width:100%">
        ...
        <el-table-column label="分户名称" min-width="240">
...
async function doQuery() {
  ...
  try {
    // 1. 获取分户树结构
    const treeR = await getTenementList(app.buildingSign, conversionType.value === 2 ? 2 : 1)
    const treeTree = treeR?.tree || []
    const hasTree = treeTree.length > 0

    // 2. 当前期能耗数据
    const params = { sign: app.buildingSign, item_ids: '', start_date: dateRange.value[0], end_date: dateRange.value[1], xdate: 'range', conversion_type: conversionType.value || 3, energy_type: 1 }
    const curR = await getTenementAnalysis(params)
    ...
    try {
      const prevR = await getTenementAnalysis({ ...params, start_date: fmt(prevSd), end_date: fmt(prevEd) })
...
.report-content { background:var(--card-bg);padding:28px;border-radius:8px }
