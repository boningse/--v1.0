...
export const useAuthStore = defineStore('auth', () => {
  const token = ref(sessionStorage.getItem('token') || ''); const user = ref<any>(null)
  function setToken(t: string) { token.value = t; sessionStorage.setItem('token', t) }
  function setUser(u: any) { user.value = u }
  function logout() { token.value = ''; user.value = null; sessionStorage.removeItem('token') }
